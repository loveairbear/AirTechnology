...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if(number == 97 ){GhostColor[] = {0,0,255};notificationflag = true;}
  ...
  if(number==21 || number==22 || number==23){
    ...
    while(true){
      ...
      if(number==21){
        matrix.print(F("xoxo"));
        if(--matrixwidth < -45) {matrixwidth = matrix.width();break;}
      }
      else if(number==22){
        matrix.print(F("FTW!!!"));
        if(--matrixwidth < -26) {matrixwidth = matrix.width();break;}
      }
      else if(number==23){
        matrix.print(F("ROFL!"));
        if(--matrixwidth < -30) {matrixwidth= matrix.width();break;}
      }
      matrix.show();
      delay(120);
      matrix.show();
  ...
  if(number==23){fetchNsketch("chat",5,7,false);}
